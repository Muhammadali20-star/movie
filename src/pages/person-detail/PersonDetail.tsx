...
const Skeleton = () => {
  return (
    <div className="flex flex-col md:flex-row gap-10 mb-12 items-center md:items-start bg-white dark:bg-slate-900 p-6 rounded-[10px] animate-pulse">
      <div className="w-[250px] h-[375px] rounded-2xl bg-gray-300 dark:bg-slate-700" />
      <div className="flex-1 space-y-4 w-full">
        ...
        <div className="space-y-2">
          ...
          <div className="h-4 bg-gray-300 dark:bg-slate-700 rounded w-11/12"></div>
        ...
        <>
          <div className="flex flex-col md:flex-row gap-10 mb-12 items-center md:items-start bg-white dark:bg-slate-900 p-6 rounded-[10px]">
            <img
              ...
              loading="lazy"
              alt={person.name}
              className="w-[250px] h-[375px] rounded-2xl object-cover shadow-md"
            />
            <div className="flex-1 space-y-4">
              <h1 className="text-4xl font-bold text-gray-900 dark:text-white">{person.name}</h1>
              <p className="text-base text-gray-600 dark:text-gray-400">
                <span className="font-medium">Mashhurligi:</span> {person.popularity?.toFixed(1)}
              </p>
              {person.biography ? (
                <p className="text-[15px] leading-6 text-gray-800 dark:text-gray-300">{person.biography}</p>
              ) : (
                <p className="italic text-gray-500 dark:text-gray-400">Biografiya mavjud emas.</p>
              )}
            </div>
          </div>
