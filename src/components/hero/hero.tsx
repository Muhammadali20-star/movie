...
const Hero: FC = () => {
  ...
  if (isLoading) {
    return (
      <>
        <div className='max-w-[1250px] h-[370px] sm:h-[530px] md:h-[590px] lg:h-[650px] rounded-xl dark:bg-slate-900 bg-white'></div>
        <div className='flex items-center justify-center gap-2 mt-4'>
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className='w-full h-[64px] sm:h-[72px] md:h-[200px] rounded-xl dark:bg-slate-900 bg-white'
            ></div>
          ))}
        </div>
      </>
    ...
    <>
        ...
        {movieData?.results?.slice(0, 5).map((movie: IMovie) => (
          <SwiperSlide key={movie.id}>
            <div className="relative h-[350px] sm:h-[520px] md:h-[580px] lg:h-[640px] rounded overflow-hidden">
              <img src={IMAGE_URL + movie.backdrop_path} alt={movie.title} loading='lazy' className="w-full h-full object-cover"/>
      ...
      <Swiper
        ...
        {movieData?.results?.slice(0, 4).map((movie: IMovie) => (
          <SwiperSlide key={movie.id}>
            <img src={IMAGE_URL + movie.backdrop_path} loading='lazy' alt={movie.title} className="w-full object-contain"/>
          </SwiperSlide>
        ))}
      </Swiper>
    </>
